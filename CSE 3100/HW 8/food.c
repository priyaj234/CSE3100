...
typedef struct {
	int size; //how many rows can be in the columns
        int buf[MAX][3]; //2d array that is the buffer, always have 3 cols, rows can be anywhere to max
        int remain; //decrement remain for each time an order is completed
        int counts[3];            //current indexes to find which column
        pthread_mutex_t mutex; //the mutex
        pthread_cond_t produce_cond; //signal that the producer can start making something
        pthread_cond_t consume_cond; //signal for the consumer to know there something there for it to take
}two_d_buffer;
...
{   
	...
	while(!done)
	{
		...
		pthread_mutex_lock(&p->mutex);
		node *created = remove_first(&p->head, &p->tail);
		pthread_mutex_unlock(&p->mutex);
		if (created == NULL){
			done = 1;
			continue;
		...
		prepare(created->v);
		add_to_buffer(created->v, created->v, q);
		my_data->total ++;
		free(created);
...
int main(int argc, char *argv[])
{
	...
	for (int i = n_consumer; i < n_consumer+n_producer; i++){
