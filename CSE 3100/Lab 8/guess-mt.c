...
void * thread_c(void * arg_in)
{
    ...
    pthread_mutex_lock(&arg -> mutex); //lock mutex
    //send max values
    arg->status = S_MAX;
    arg->max = gmn_get_max();
    ...
    do{
        ...
        pthread_mutex_unlock(&arg->mutex);
        int guess_result = gmn_check(&gmn, arg->guess);
    ...
    } while(arg->status != S_GUESS); //      wait for a guess from thread_p 
         //   call gmn_check()
...
int main(int argc, char *argv[])
{
    ...
    // 2x
    pthread_create(&one, NULL, thread_c, &arg);
    pthread_create(&two, NULL, thread_p, &arg);
