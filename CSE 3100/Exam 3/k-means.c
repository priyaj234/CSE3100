...
void* thread_main(void* thread_arg) {
    ...
    clusters(a, labels, m, k, 1, rounds);
    squared_intra_cluster_dist(centroids, k, a, m, labels);
    compute_centroids(centroids, k, a, m, labels);
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < n_threads; i++){
        pthread_create(&thread_arr[i], NULL, thread_main, NULL);
    }
    for(int i = 0; i < n_threads; i++){
    ...
    double best_d = clusters(a, labels, m, k, trials, rounds);
