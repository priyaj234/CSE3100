...
void start_program(Program *programs, int num_programs, int cur) 
{
    ...
    if(child == 0){
        ...
        if(cur != (num_programs -1)){
            for(int i = cur + 1; i < (num_programs - 1); i++){
                close(programs[i].fd_in);
                close(programs[i].fd_out);
            }
            close(programs[num_programs -1].fd_in);
        }
...
void prepare_pipes(Program *programs, int num_programs)
{
    ...
    for(int i = 0; i < (num_programs - 1); i++){
        ...
        if (pipe(pipes) == -1){
            perror("pipe()");
            die("pipe function failed");
        }
        programs[i].fd_out = pipes[PIPEFD_WRITE];
        programs[i + 1].fd_in = pipes[PIPEFD_READ];

    }
    programs[num_programs - 1].fd_out = PIPEFD_WRITE;
}
