...
{
    printer_t *printers = arg;

    printers->njobs = 0;
    while(q_num_jobs(printers->jq) != 0){
        pthread_mutex_lock(&printers->jq->mutex);
        int num = q_fetch_job(printers->jq, printers->id);
        pthread_mutex_unlock(&printers->jq->mutex);
        print_job(num);
        printers->njobs += 1;
    }
    ...
    //Thread main 
     //Lock first so first doesn't get taken over builtin lock()
    return arg;
}
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < num_printers; i++){
        ...
        check_pthread_return(status, "You failed!");

    }
    for (int i = 0; i < num_printers; i++){
        status = pthread_join(printers[i].thread_id, NULL);
