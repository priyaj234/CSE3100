...
void * thread_player(void *arg_in)
{
    ...
    for (int i = 0; i < arg->n_rounds; i ++ ) {
        ...
        //Status 1 means we are waiting on ref
        while((*choice).status==1){
            pthread_cond_wait(&(*choice).cond, &(*choice).mutex);
        }
        //reached here means time to generate  choice
        int r = MAKE_CHOICE(); 

        if (! arg->opt_quiet)
            printf("Player %d: round %d: %d\n", arg->id, i, r);

        (*choice).status = 1; //set status to 1 to let ref know we have result
        (*choice).value = r;
        //unlock and wait for ref and all players to hit now 
        pthread_mutex_unlock(&(*choice).mutex);
        pthread_cond_signal(&(*choice).cond);
        pthread_barrier_wait(&(*result).barrier); 
        //after reaching barrier read outcome from ref
        int outcome = (*result).value;
        //save the result to the player 
        if (outcome == 0) 
            arg->n_ties ++;
...
void * thread_referee(void *arg_in)
{
    ...
    for (int i = 0; i < arg->n_rounds; i ++) {
        ...
        //when players ready lock threads
        //pthread_mutex_lock(&(*p1).mutex);
        //pthread_mutex_lock(&(*p2).mutex);
        int outcome = compare_choices((*p1).value, (*p2).value);
        //set status to 0 to let players know 
        (*p1).status = 0;
        (*p2).status = 0;
        //write the result to the shared variable 
        ...
        //hit the barrier so the players can read the results
        pthread_barrier_wait(&(*result).barrier); 
    }
    return NULL;
}
